...
(function() {
    ...
    class MENU extends HTMLElement {
        ...
        onCustomWidgetAfterUpdate(changedProperties) {
            ...
            if ("showGroupHeader" in changedProperties) {
                this._props.showGroupHeader = changedProperties["showGroupHeader"];
                this._selectionEvent = false;
            }
            if ("menuBackgroundColor" in changedProperties) {
                this._props.menuBackgroundColor = changedProperties["menuBackgroundColor"];
                this._selectionEvent = false;
            }
            if ("menuFontColor" in changedProperties) {
                this._props.menuFontColor = changedProperties["menuFontColor"];
                this._selectionEvent = false;
            }
        ...
        drawMenu(dataJSON, config, root, eventDispatcher) {
            ...
            var _d = config.data;
            var _menuL0 = _div.append("ul").attr("id", "menu").style('background-color', config.menuBackgroundColor);
        	
        	for(var i=0; i<_d.length; i++){
        		const node = _menuL0.append('li');
        		node.on('mouseover', function(){
        			d3.select(this).select('a').style('color', 'rgba(0, 223, 252, 1)');
        		}).on('mouseout', function(){
        			d3.select(this).select('a').style('color', config.menuFontColor);
        		});
        		const anchornode = node.append('a').attr("id", _d[i].id)
        			.style('cursor', 'pointer')
        			.style('color', config.menuFontColor);
