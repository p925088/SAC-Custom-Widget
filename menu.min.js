    ...
    class MENU extends HTMLElement {
        ...
        */
        drawChart(_menudata, config) {
        	...
        	_menudata = JSON.parse(_menudata);
        	console.log(_menudata.constructor);
            ...
            var _menuL0 = _div.appendChild(document.createElement("ul"));
        	_menuL0.setAttribute("id", "menu");
        	
        	for(var i=0; i<_menudata.length; i++){        		
        		const node = document.createElement("li");
        		const anchornode = document.createElement("a");
        		//anchornode.setAttribute("href", "#");
        		anchornode.setAttribute("id", _menudata[i].id);
        		const textnode = document.createTextNode(_menudata[i].label);
        		anchornode.appendChild(textnode);
        		anchornode.onclick = function (d){
        			console.log(d.srcElement.innerText);
        			console.log(d.srcElement.id);
        			updateSelection(d.srcElement.innerText, d.srcElement.id);
        		};
        		node.appendChild(anchornode);
        		const _submenu = _menudata[i].value;
        		//console.log(_menudata[i]);
        		//console.log(_submenu);
        		if(_submenu.length > 1 && _submenu != undefined){
        			var slist = node.appendChild(document.createElement("ul"));			
        			for(var j=0; j<_submenu.length; j++){
        				const snode = document.createElement("li");
        				const sanchornode = document.createElement("a");
        				//sanchornode.setAttribute("href", "#");
        				sanchornode.setAttribute("id", _submenu[j].id);
        				const textsnode = document.createTextNode(_submenu[j].label);
        				sanchornode.appendChild(textsnode);
        				sanchornode.onclick = function (d){
        					console.log(d.srcElement.innerText);
        					console.log(d.srcElement.id);
        					updateSelection(d.srcElement.innerText, d.srcElement.id);
        				};
        				snode.appendChild(sanchornode);        				
        				slist.appendChild(snode);
        			}	
        		}        		
        		_menuL0.appendChild(node);
        	}
        	
        	let updateSelection = function(label, id){
