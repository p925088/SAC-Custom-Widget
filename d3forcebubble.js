(function() {
	...
	template.innerHTML = `
		<div>
			<svg width=100% height=100% id=d3forcebubble>
			</svg>
		</div>
		<style>
			.circleDefault{
			...
			.topBar > table > tr > td:nth-child(2){
				text-align: right;
			}
	...
	class FBUBBLE extends HTMLElement {
		...
		d3ForceBubbleDefaultSettings(){
		    return {
		        ...
		        legendMargin: 50, // Margin for the legend.
		        title: 'Custom Force Bubble Widget', // Chart Title
		        showTitle: true, // Display or hide title
		        showAvg: true, // Display or hide average line
		        showSearch: true, //Show or hide search field
		        startColor: "#ffcd00", // Color for maximum value.
		        endColor: "#b01c02", // Color for minimum value.
		        sizeDecimal: 2, // Round size value up to N decimal places.
		        colorDecimal: 2, // Round color value up to N decimal places.
		        valDecimal: 2, // Round primary value up to N decimal places.
		        xAxisLabel: "Value", // Label for measure determining size
		        sizeLabel: "Size", // Label for measure determining size
		        colorLabel: "Color", // Label for measure determining color
		        selectedLabel: undefined, // Selected label when a bubble is selected
		        selectedXValue: undefined, //Selected x-axis value when a bubble is selected
		        selectedSizeValue: undefined, //Selected size value when a bubble is selected
		        selectedColorValue: undefined, //Selected color value when a bubble is selected
		        data : [] //Data in JSON format
		    };
		}
		
		/* ---------- Force Bubble Component ---------- */
		drawForceBubble(dataJSON, config, root, eventDispatcher){			
			...
			if(config.showSearch){
				...
				d3.select(searchcontainer).append('input')
				.attr('id','searchfield')
				.attr('type','search')
			...
			var vis = container.append('div').attr('class','chartBody')
			...
			var min_x = _.minBy(nodes, function(d){ return d.xvalue; }).xvalue;
			var max_x = _.maxBy(nodes, function(d){ return d.xvalue; }).xvalue;
			var mean_x = _.meanBy(nodes, function(d){ return d.xvalue; });
			var min_c = _.minBy(nodes, function(d){ return d.color; }).color;
			var max_c = _.maxBy(nodes, function(d){ return d.color; }).color;
			var min_s = _.minBy(nodes, function(d){ return d.size; }).size;
			var max_s = _.maxBy(nodes, function(d){ return d.size; }).size;
			...
			sizeSVG.append('text').attr('x',65).attr('y',8).text(formatSizeValue(max_s))
				.attr("font-family", "Arial, Helvetica, sans-serif")
				.attr("font-size", "10px")
				.attr("fill", "#999");
			sizeSVG.append('text').attr('x',65).attr('y',48).text(formatSizeValue(min_s))
			.attr("font-family", "Arial, Helvetica, sans-serif")
			.attr("font-size", "10px")
			.attr("fill", "#999");			
			...
			colorSVG.append('text').attr('x',30).attr('y',30).text(formatColorValue(min_c))
			.attr("font-family", "Arial, Helvetica, sans-serif")
			.attr("font-size", "10px")
			.attr("fill", "#999").style('text-anchor','middle');
			colorSVG.append('text').attr('x',110).attr('y',30).text(formatColorValue(max_c))
			.attr("font-family", "Arial, Helvetica, sans-serif")
			.attr("font-size", "10px")
			.attr("fill", "#999").style('text-anchor','middle');
			...
			if(config.showAvg){
				vis_g.append('line').attr('x1', 0 + xScale(mean_x))
				.attr('y1', -50)
